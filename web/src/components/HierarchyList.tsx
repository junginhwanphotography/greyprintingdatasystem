...
interface HierarchyListProps {
  ...
  onDuplicateItem?: (item: HierarchyItem) => Promise<void>;
  emptyMessage?: string;
}
...
export default function HierarchyList({
  ...
  const handleDuplicate = async (item: HierarchyItem) => {
                      ...
                      <div className="absolute right-0 top-full z-20 mt-1 w-36 rounded-lg border border-border bg-surface py-1 shadow-lg">
                        ...
                        {onDuplicateItem && (
                          <button
                            ...
                            className="w-full px-3 py-2 text-left text-sm text-foreground hover:bg-zinc-50 disabled:opacity-50"
                          >
                            복사
                          </button>
                        )}
